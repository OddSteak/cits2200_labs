...
class Edge:
    def __init__(self, node: Node, time: int, clearance: Clearance):
        self.node = node
        self.time = time
...
class Node:
    def __init__(self, key: int | float, count: int, value: int, clearance: Clearance):
        self.key = key
        self.count = count
        self.value = value
        self.clearance = clearance
    ...
    def addEdge(self, node: Node, time: int, clearance: Clearance):
...
class AdaptableHeapPriorityQueue:
    ...
    def add(self, key: int | float, value: int, clearance: Clearance) -> int:
        ...
        item = Node(key, self._counter, value, clearance)
    ...
    def update(self, value: int, new_key: int):
        """update the key of the element with value"""
        if value not in self._locator:
            raise KeyError("key not in priority queue")
        node = self.getItem(value)
        node.key = new_key

    def restoreHeap(self):
